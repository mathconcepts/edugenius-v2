...
const ATTRIBUTES: UserAttribute[] = [
...

  {
    ...
    dataType: 'enum', validation: 'free | pro | premium | elite',
    purpose: 'Gates all features including chatbot channel access. free=portal only; pro=portal+add-ons; premium=portal+WhatsApp+Telegram+Meet(2); elite=all+Meet(4).',
    fallback: 'Free plan defaults. Chatbot channels unavailable.',
    example: 'pro',
    impacted: [
      {name:'Sage',                kind:'agent',    emoji:'🧠', impact:'critical', reason:'AI chat session limit AND chatbot channel access both enforced by plan'},
      {name:'ChatbotAccessService',kind:'subagent',            impact:'critical', reason:'Central plan-gating layer — checks plan+add-ons before every WhatsApp/Telegram message'},
      {name:'Mentor',              kind:'agent',    emoji:'🎯', impact:'critical', reason:'Outreach routing skips WhatsApp/Telegram if plan does not allow'},
      {name:'Herald',              kind:'agent',    emoji:'📣', impact:'high',     reason:'Campaign delivery restricted to allowed channels per plan'},
      {name:'RevenueArchitect',    kind:'agent',    emoji:'💰', impact:'high',     reason:'UpSellEngine targets free/pro users without chatbot access for upgrade campaigns'},
      {name:'Atlas',               kind:'agent',    emoji:'📚', impact:'high',     reason:'Question download quota per plan'},
      {name:'Paywall / Feature Gate', kind:'feature',           impact:'critical', reason:'All premium features and all chatbot channels gated on plan'},
      {name:'Subscription Manager',   kind:'subagent',          impact:'critical', reason:'Plan enforcement and renewal management'},
      {name:'Revenue Tracker',        kind:'subagent',          impact:'high',     reason:'MRR/ARR calculation by plan + add-on revenue tracking'},
    ],
  },
  {
    id: 'active_add_ons', name: 'Active Chatbot Add-ons', category: 'Subscription & Payment', status: 'optional',
    roles: ['student'], collectAt: ['payment','settings'],
    dataType: 'array', validation: 'chatbot_whatsapp | chatbot_telegram | chatbot_all',
    purpose: 'Pro-plan students can purchase chatbot channel access as monthly add-ons (₹99/mo each or ₹149/mo bundle). Add-on status used by ChatbotAccessService.',
    fallback: 'No chatbot add-ons active — portal-only access.',
    example: '["chatbot_all"]',
    impacted: [
      {name:'ChatbotAccessService',kind:'subagent',            impact:'critical', reason:'Reads add-ons alongside plan to determine channel entitlement'},
      {name:'Mentor',              kind:'agent',    emoji:'🎯', impact:'high',     reason:'Activates WhatsApp/Telegram outreach when add-on is present'},
      {name:'RevenueArchitect',    kind:'agent',    emoji:'💰', impact:'high',     reason:'Tracks add-on MRR; identifies cross-sell opportunities'},
      {name:'Oracle',              kind:'agent',    emoji:'📊', impact:'medium',   reason:'Reports chatbot adoption rates and add-on revenue by cohort'},
      {name:'Settings/Billing',    kind:'feature',             impact:'high',     reason:'User can manage add-ons from Settings → Channels tab'},
    ],
  },
  {
    id: 'whatsapp_number', name: 'WhatsApp Number (verified)', category: 'Subscription & Payment', status: 'conditional',
    conditionNote: 'Required when plan includes WhatsApp or chatbot_whatsapp add-on is active',
    roles: ['student','parent'], collectAt: ['channel_connect','settings'],
    dataType: 'phone', validation: 'E.164 format e.g. +919876543210; OTP-verified',
    purpose: 'Routes AI tutor interactions through WhatsApp. Sage and Mentor use this to deliver personalised responses on the student preferred channel.',
    fallback: 'WhatsApp interaction unavailable. Falls back to in-app notifications.',
    example: '+919876543210',
    impacted: [
      {name:'ChatbotAccessService',kind:'subagent',            impact:'critical', reason:'Checks whatsappVerified before allowing any WA message delivery'},
      {name:'Sage',                kind:'agent',    emoji:'🧠', impact:'critical', reason:'Tutoring sessions delivered via WhatsApp when verified'},
      {name:'Mentor',              kind:'agent',    emoji:'🎯', impact:'critical', reason:'Daily reminders, streak alerts, exam countdowns via WhatsApp'},
      {name:'Herald',              kind:'agent',    emoji:'📣', impact:'high',     reason:'Promotional campaigns and announcements routed here'},
      {name:'Nexus',               kind:'agent',    emoji:'🔗', impact:'medium',   reason:'Support ticket updates sent via WhatsApp if verified'},
    ],
  },
  {
    id: 'telegram_user_id', name: 'Telegram User ID (verified)', category: 'Subscription & Payment', status: 'conditional',
    conditionNote: 'Required when plan includes Telegram or chatbot_telegram add-on is active',
    roles: ['student','teacher','parent'], collectAt: ['channel_connect','settings'],
    dataType: 'string', validation: 'Numeric Telegram user ID — verified via /link bot command',
    purpose: 'Routes AI tutor interactions through Telegram. Sage delivers rich-media explanations, inline quizzes, and formula rendering in Telegram.',
    fallback: 'Telegram interaction unavailable. Falls back to in-app.',
    example: '1234567890',
    impacted: [
      {name:'ChatbotAccessService',kind:'subagent',            impact:'critical', reason:'Checks telegramVerified before any Telegram message delivery'},
      {name:'Sage',                kind:'agent',    emoji:'🧠', impact:'critical', reason:'Full tutoring via Telegram — rich text, LaTeX, quizzes'},
      {name:'Mentor',              kind:'agent',    emoji:'🎯', impact:'high',     reason:'Daily nudges and milestone messages via Telegram'},
      {name:'Herald',              kind:'agent',    emoji:'📣', impact:'medium',   reason:'Exam alerts and content announcements via Telegram'},
      {name:'Forge',               kind:'agent',    emoji:'⚙️', impact:'high',     reason:'Configures Telegram webhook and bot routing on channel connect'},
    ],
  },
  {
